...
{   
     ...
     * (implementation assumed node exists)
     * Pre-condition: the node is neither the head nor the tail of the list.
     * @param node - middle node to be removed
     */
    protected void removeMiddleNode( DoubleListNode<E> node ) //throws EmptyListException
    {   
    	//if(this.isEmpty()) throw new EmptyListException();
    ...
    protected DoubleListNode<E> findNode( E element )
    {
    	...
    	while(current != null)
    	{
    		if(current.getElement() == element) return current;
    ...
    {
        DoubleListNode<E> otherHead = list.head;
        
        otherHead.setPrevious(this.tail);
        this.tail.setNext(otherHead);
        this.tail = list.tail;
        currentSize += list.currentSize;
        
        list.head = null; //erase other list 
        list.tail = null;
        list.currentSize = 0;
    }
